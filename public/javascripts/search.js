(function () {
  function giphySearch(keyword) {
    return fetch(`https://api.giphy.com/v1/gifs/search?q=${keyword}&api_key=${key}`)
      .then(response => response.json());
  }
	
  function stickerSearch(keyword) {
    return fetch(`https://api.giphy.com/v1/stickers/search?q=${keyword}&api_key=${key}`)
      .then(response => response.json());
  }
  ...
  function appendImage2(img) {
	  let $div = $('<div class="img-wrapper2"></div>');
